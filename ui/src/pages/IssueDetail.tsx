...
export default function IssueDetail({ issueId, onBack }: Props) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      <button onClick={onBack} className="text-sm text-blue-600 hover:underline mb-5">&larr; Back to issues</button>
      ...
      <div className="card overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 flex items-center gap-3">
          ...
          <span className="text-xs font-mono text-gray-500">{issue.scanner}</span>
          <span className="text-xs text-gray-400">{issue.rule}</span>
        </div>

        <div className="px-6 py-5">
          <p className="text-base font-medium text-gray-800 mb-3">{issue.message}</p>

          <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
            <span className="font-mono text-blue-600">{issue.file}</span>
            <span>:</span>
          ...
          {issue.solution && (
            <div className="mb-6">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">How to fix it</p>
              <div className="bg-blue-50 border border-blue-200 rounded p-4 text-sm leading-relaxed text-gray-700">
                {issue.solution}
              </div>
            </div>
          )}
          ...
          <div className="border-t border-gray-200 pt-4">
            {canFix ? (
              ...
              <p className="text-sm text-gray-400">Auto-fix not available for this issue type.</p>
            )}
            {fixResult && (
