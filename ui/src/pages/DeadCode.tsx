  ...
  return (
    <div className="max-w-6xl mx-auto p-8">
      <h2 className="text-lg font-bold text-gray-800 dark:text-ctp-text mb-5">
        Dead Code <span className="font-normal text-gray-400 dark:text-ctp-overlay1">({deadIssues.length})</span>
      </h2>

      {deadIssues.length === 0 ? (
        <div className="card p-8 text-center">
          <p className="text-gray-500 dark:text-ctp-overlay0">No dead code issues found.</p>
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-gray-200">
              {deadIssues.map((issue) => (
                <tr key={issue.id} className="hover:bg-gray-50 dark:hover:bg-ctp-surface0 transition-colors group">
                  ...
                  <td className="px-5 py-3 font-mono text-xs text-gray-500 dark:text-ctp-overlay1">{issue.rule}</td>
                  ...
                  <td className="px-5 py-3">
                    <button
                      ...
                      className="text-xs font-mono text-gray-500 hover:text-blue-600 dark:text-ctp-overlay1 dark:hover:text-ctp-blue"
                    >
                      {issue.file}
                    </button>
                  </td>
