  ...
  return (
    <div className="max-w-6xl mx-auto p-8">
      <h2 className="text-lg font-bold text-gray-800 dark:text-ctp-text mb-5">
        Security <span className="font-normal text-gray-400 dark:text-ctp-overlay1">({securityIssues.length})</span>
      </h2>

      {securityIssues.length === 0 ? (
        <div className="card p-8 text-center">
          <p className="text-gray-500 dark:text-ctp-overlay0">No security issues found.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50 dark:bg-ctp-mantle">
                ...
                <th className="text-left px-5 py-2.5 font-medium text-gray-500 dark:text-ctp-overlay0 text-xs uppercase tracking-wide">Scanner</th>
                <th className="text-left px-5 py-2.5 font-medium text-gray-500 dark:text-ctp-overlay0 text-xs uppercase tracking-wide">Rule</th>
                <th className="text-left px-5 py-2.5 font-medium text-gray-500 dark:text-ctp-overlay0 text-xs uppercase tracking-wide">Description</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {securityIssues.map((issue) => (
                <tr key={issue.id} onClick={() => onSelectIssue?.(issue.id)} className="hover:bg-gray-50 dark:hover:bg-ctp-surface0 transition-colors cursor-pointer">
                  ...
                  <td className="px-5 py-3 text-gray-500 dark:text-ctp-overlay0 font-mono text-xs">{issue.scanner}</td>
                  ...
                  <td className="px-5 py-3 text-gray-400 dark:text-ctp-overlay1 font-mono text-xs text-right">{issue.line > 0 ? issue.line : '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
